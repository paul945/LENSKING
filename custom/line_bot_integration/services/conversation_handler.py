...
歡迎隨時聯絡我們！"""
    ...
    def _send_category_menu_v3(self, line_user, reply_token):
        ...
        try:
            ...
            if not categories:
                # 降級到舊版本
                _logger.info('無法從 Odoo 讀取分類，降級到舊版本')
                self._send_category_menu(line_user, reply_token)
                return
            ...
            line_client.reply_message(reply_token, messages)
            self.env['line.conversation'].log_outgoing_message(line_user, 'flex', '器材分類選單 v3')
            
            _logger.info(f'成功發送 v3 分類選單，共 {len(categories)} 個分類')
            
        except Exception as e:
            _logger.error(f'v3 分類選單失敗，降級到舊版本：{str(e)}', exc_info=True)
            self._send_category_menu(line_user, reply_token)
        ...
        try:
            ...
            if not equipment_list:
                ...
                messages = [{
                    ...
                    'quickReply': {'items': [{
                        'type': 'action',
                        'action': {'type': 'message', 'label': '返回分類', 'text': '租借器材'}
                    }]}
                }]
                line_client.reply_message(reply_token, messages)
                return
            ...
            line_client.reply_message(reply_token, messages)
            self.env['line.conversation'].log_outgoing_message(line_user, 'flex', f'{category.name}器材列表 v3')
            
            _logger.info(f'成功顯示分類 {category.name} 的 {len(bubbles)} 個產品')
            
        except Exception as e:
            _logger.error(f'v3 產品列表失敗：{str(e)}', exc_info=True)
            # 降級到錯誤訊息
            text = '抱歉，載入產品時發生錯誤。請稍後再試。'
            messages = [{'type': 'text', 'text': text}]
            line_client.reply_message(reply_token, messages)
    ...
    def _add_to_cart_v3(self, line_user, equipment_id, reply_token):
        ...
        try:
            ...
            if not equipment:
                # 降級到舊版本
                _logger.info(f'產品 {equipment_id} 不存在，嘗試舊版本')
...
💵 小計：NT$ {int(total)}"""
            ...
            _logger.info(f'成功加入產品 {equipment["name"]} 到購物車')
            
        except Exception as e:
            _logger.error(f'v3 加入購物車失敗：{str(e)}', exc_info=True)
            # 降級到舊版本
            self._add_to_cart(line_user, equipment_id, reply_token)
